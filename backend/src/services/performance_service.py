...
class PerformanceService:
    ...
    def __init__(self, db_manager: DatabaseManager, exchange_name: str = "binanceusdm"):
    ...
    async def get_performance_summary(
        ...
        try:
            ...
            async with self.db_manager.get_session() as session:
                ...
                if all_metrics:
                    return await self._aggregate_metrics_with_snapshots(all_metrics, start_date, end_date)
                else:
    ...
    async def _aggregate_metrics_with_snapshots(
        ...
        try:
            async with self.db_manager.get_session() as session:
                ...
                # 如果metrics_list为空，从closed_positions重新计算交易统计
                if not metrics_list:
                    closed_positions = await dao.get_closed_positions(
                        start_date=start_date,
                        end_date=end_date,
            ...
            self.logger.warning(f"计算收益率失败: {e}")
